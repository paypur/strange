...
public class StrangeItemTagsProvider extends ItemTagsProvider {

    public StrangeItemTagsProvider(DataGenerator pGenerator, @Nullable ExistingFileHelper existingFileHelper) {
    ...
    protected void addTags() {
        ...
        final TagAppender<Item> armor = this.tag(Strange.ARMOR);
        ...
        for (Item item: ForgeRegistries.ITEMS.getValues()) {
            if (ItemTypeUtil.isWeapon(item) && !(item instanceof AxeItem)) {
                ...
                tools.add(item);
            } else if (ItemTypeUtil.isDefense(item)) {
                armor.add(item);
            }
        }
    }
